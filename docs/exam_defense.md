...
The relation labels in Dataset V4 are generated deterministically from the documented image category, text category and functional-family mapping. This defines the benchmark ground truth; the relations are not manually annotated pair by pair. Importantly, these category and family metadata fields are not provided to the relation classifier as inference inputs. The model must infer useful information from the image representation and the text representation and then predict the relation between them.

The auxiliary category targets are used only as additional supervision during training. The total training objective combines the main relation loss with image-category and text-category losses, each weighted by `0.15`. The ground-truth category labels are not concatenated with the model inputs and are not supplied to the relation head at inference time. The auxiliary tasks therefore encourage more informative modality representations rather than providing the final relation answer directly.

In the official Dataset V4 validation and final-test workflow, the ImageNet-pretrained ResNet18 backbone is used in evaluation mode as a frozen feature extractor. Its 512-dimensional image features are computed and cached, and backpropagation updates only the downstream trainable neural classifier and auxiliary heads. Caching the frozen features reduces repeated computation while preserving the same fixed visual representation throughout the controlled experiment.

The final classifier outputs three logits, one for each relation class. These are raw, unnormalized scores. `CrossEntropyLoss` is appropriate because every image-text pair has exactly one correct class among three mutually exclusive classes, and PyTorch applies the required log-softmax behavior internally. A softmax transformation can be used when the class scores need to be displayed as normalized values that sum to one, for example in the practical visualization. These scores can be interpreted as model confidence for comparison between classes, but without a separate calibration analysis they should not be treated as guaranteed real-world probabilities.
